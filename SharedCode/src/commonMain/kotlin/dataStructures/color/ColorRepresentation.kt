...
import com.rohengiralt.debatex.xkcdException
import kotlinx.serialization.Serializable
import kotlin.math.abs
import kotlin.reflect.KClass

@Suppress("Unused")
inline fun <reified T : ColorRepresentation> ColorRepresentation.coerce(): T {
    return coerce(T::class)
}
    ...
    data class RGB(
        ...
        override fun <T : ColorRepresentation> coerce(representation: KClass<T>): T =
            when (representation) {
                ...
                else -> if (/*representation in ColorRepresentation::class.sealedSubclasses*/true) { //TODO: Find workaround or change if reflections are brought to K/N
                    logger.warn("ColorRepresentation RGB was unable to convert to ${representation.simpleName}")
                    throw IllegalArgumentException("Cannot convert from RGB to ${representation.simpleName}")
                } else throw xkcdException()
            } as T

        companion object {
            private val logger = loggerForClass<RGB>()
        }
    }
    ...
    data class HSV(
        ...
        override fun <T : ColorRepresentation> coerce(representation: KClass<T>): T =
            when (representation) {
                ...
                ColorRepresentation::class -> this
                else -> if (/*representation in ColorRepresentation::class.sealedSubclasses*/true) {
                    logger.warn("ColorRepresentation HSV was unable to convert to ${representation.simpleName}")
                    throw IllegalArgumentException("Cannot convert from HSV to ${representation.simpleName}")
                } else throw xkcdException()
            } as T

        companion object {
            private val logger = loggerForClass<HSV>()
        }
    }
        ...
        override fun <T : ColorRepresentation> coerce(representation: KClass<T>): T =
            when (representation) {
                ...
                ColorRepresentation::class -> this
                    ...
                    logger.warn("ColorRepresentation HSL was unable to convert to ${representation.simpleName}")
                    throw IllegalArgumentException("Cannot convert from HSL to ${representation.simpleName}")
                } else throw xkcdException()
            } as T

        companion object {
            private val logger = loggerForClass<HSL>()
        }
    }
